...
import { RestaurantsViewDialog } from '@/components/dialogs/RestaurantsViewDialog';
...
const RESTAURANTS_FIELDS = [
  { key: 'name', label: 'Name des Restaurants', type: 'string/text' },
  { key: 'beschreibung', label: 'Kurzbeschreibung', type: 'string/textarea' },
  ...
  { key: 'oeffnungszeiten_mo', label: 'Montag', type: 'string/text' },
  { key: 'oeffnungszeiten_di', label: 'Dienstag', type: 'string/text' },
  { key: 'oeffnungszeiten_mi', label: 'Mittwoch', type: 'string/text' },
  { key: 'oeffnungszeiten_do', label: 'Donnerstag', type: 'string/text' },
  { key: 'oeffnungszeiten_fr', label: 'Freitag', type: 'string/text' },
  { key: 'oeffnungszeiten_sa', label: 'Samstag', type: 'string/text' },
  { key: 'oeffnungszeiten_so', label: 'Sonntag', type: 'string/text' },
  { key: 'kuechenstil', label: 'Küchenstil', type: 'lookup/select', options: [{ key: 'italienisch', label: 'Italienisch' }, { key: 'asiatisch', label: 'Asiatisch' }, { key: 'deutsch', label: 'Deutsch' }, { key: 'griechisch', label: 'Griechisch' }, { key: 'indisch', label: 'Indisch' }, { key: 'japanisch', label: 'Japanisch' }, { key: 'mexikanisch', label: 'Mexikanisch' }, { key: 'amerikanisch', label: 'Amerikanisch' }, { key: 'franzoesisch', label: 'Französisch' }, { key: 'tuerkisch', label: 'Türkisch' }, { key: 'mediterran', label: 'Mediterran' }, { key: 'vietnamesisch', label: 'Vietnamesisch' }, { key: 'thailaendisch', label: 'Thailändisch' }, { key: 'spanisch', label: 'Spanisch' }, { key: 'sonstige', label: 'Sonstige' }] },
  { key: 'ernaehrungsfilter', label: 'Ernährungsfilter', type: 'multiplelookup/checkbox', options: [{ key: 'vegan', label: 'Vegan' }, { key: 'vegetarisch', label: 'Vegetarisch' }, { key: 'glutenfrei', label: 'Glutenfrei' }] },
  { key: 'foto', label: 'Foto des Restaurants', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'restaurants', label: 'Restaurants', pascal: 'Restaurants' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('restaurants');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'restaurants': return (data as any).restaurants as Restaurants[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'restaurants': return RESTAURANTS_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'restaurants': return {
        create: (fields: any) => LivingAppsService.createRestaurant(fields),
        update: (id: string, fields: any) => LivingAppsService.updateRestaurant(id, fields),
        remove: (id: string) => LivingAppsService.deleteRestaurant(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'restaurants' || dialogState?.entity === 'restaurants') && (
        <RestaurantsDialog
          open={createEntity === 'restaurants' || dialogState?.entity === 'restaurants'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'restaurants' ? handleUpdate : (fields: any) => handleCreate('restaurants', fields)}
          defaultValues={dialogState?.entity === 'restaurants' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Restaurants']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Restaurants']}
        />
      )}
      {viewState?.entity === 'restaurants' && (
        <RestaurantsViewDialog
          open={viewState?.entity === 'restaurants'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'restaurants', record: r }); }}
        />
      )}
