...
import type { Restaurants, CreateRestaurants } from '@/types/app';
...
export class LivingAppsService {
  // --- RESTAURANTS ---
  static async getRestaurants(): Promise<Restaurants[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.RESTAURANTS}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Restaurants[];
    return enrichLookupFields(records, 'restaurants');
  }
  static async getRestaurant(id: string): Promise<Restaurants | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.RESTAURANTS}/records/${id}`);
    const record = { record_id: data.id, ...data } as Restaurants;
    return enrichLookupFields([record], 'restaurants')[0];
  }
  static async createRestaurant(fields: CreateRestaurants) {
    return callApi('POST', `/apps/${APP_IDS.RESTAURANTS}/records`, { fields: cleanFieldsForApi(fields as any, 'restaurants') });
  }
  static async updateRestaurant(id: string, fields: Partial<CreateRestaurants>) {
    return callApi('PATCH', `/apps/${APP_IDS.RESTAURANTS}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'restaurants') });
  }
  static async deleteRestaurant(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.RESTAURANTS}/records/${id}`);
  }
